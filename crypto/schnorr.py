...
class SchnorrSigner(Signer):
    ...
    def __init__(self, key: int, q: int, g: int, hash_func: Callable[[str, str], int]):
        """
    ...
    def sign(self, data: str) -> SchnorrSignature:
        ...
        :return: signature.
        """
        k = randint(1, self._q - 1)
        r = pow(self._g, k, self._q)  # g^k mod q
        e = self._hash_func(str(r), data) % self._q
        s = (k - (self._key * e)) % (self._q - 1)
...
class SchnorrVerifier(Verifier):
    ...
    def __init__(self, keys: Union[int, Sequence[int]], q: int, g: int, hash_func: Callable[[str, str], int]):
        """
    ...
    def _verify_one(self, key: int, data: str, signature: SchnorrSignature) -> bool:
        ...
        :return: True if the signature was verified, False otherwise.
        """
        rv = (pow(self._g, signature.s, self._q) * pow(key, signature.e, self._q)) % self._q
        ev = self._hash_func(str(rv), data) % self._q
